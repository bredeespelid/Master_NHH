...
# New Variable for Linearization
var small >= 0;  # Auxiliary variable to represent the maximum difference in travel distances
var total_travel >= 0;
var max_travel >= 0;
var min_travel >= 0;
var slack_total_travel >= 0;
var slack_max_min_diff >= 0;

#____________________________________________ Objective

# Objective: Minimize slack variables for both objectives
minimize WeightedObjective:
    slack_total_travel + slack_max_min_diff;
#____________________________________________ Constraints

# Constraints for Maximum Difference (Linearized)

# Constraints for Total Travel
subject to TotalTravelUpper:
    total_travel <= 13503 + slack_total_travel;

subject to TotalTravelLower:
    total_travel >= 13503 - slack_total_travel;

# Constraints for Max-Min Difference
subject to MaxMinDiffUpper:
    max_travel - min_travel <= 696 + slack_max_min_diff;

subject to MaxMinDiffLower:
    max_travel - min_travel >= 696 - slack_max_min_diff;

# Calculate total travel
subject to TotalTravelConstraint:
    total_travel = sum{t in TEAMS} travel[t];
