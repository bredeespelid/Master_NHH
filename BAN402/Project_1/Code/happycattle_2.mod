...
subject to production_balance {p in PRODUCTS}:
    production[p] = sum {r in RAW_MATERIALS} blend[p,r] + new_wheat_usage[p];
...
subject to new_wheat_supply_constraint:
    sum {p in PRODUCTS} new_wheat_usage[p] <= new_wheat_supply;

subject to min_protein_constraint {p in PRODUCTS}:
    sum {r in RAW_MATERIALS} (protein[r] * blend[p,r]) + protein['wheat'] * new_wheat_usage[p] >= min_protein[p] * production[p];

subject to max_protein_constraint {p in PRODUCTS: max_protein[p] < Infinity}:
    sum {r in RAW_MATERIALS} (protein[r] * blend[p,r]) + protein['wheat'] * new_wheat_usage[p] <= max_protein[p] * production[p];

subject to min_carb_constraint {p in PRODUCTS}:
    sum {r in RAW_MATERIALS} (carb[r] * blend[p,r]) + carb['wheat'] * new_wheat_usage[p] >= min_carb[p] * production[p];

subject to max_carb_constraint {p in PRODUCTS}:
    sum {r in RAW_MATERIALS} (carb[r] * blend[p,r]) + carb['wheat'] * new_wheat_usage[p] <= max_carb[p] * production[p];

subject to min_vitamin_constraint {p in PRODUCTS}:
    sum {r in RAW_MATERIALS} (vitamin[r] * blend[p,r]) + vitamin['wheat'] * new_wheat_usage[p] >= min_vitamin[p] * production[p];

subject to max_vitamin_constraint {p in PRODUCTS: max_vitamin[p] < Infinity}:
    sum {r in RAW_MATERIALS} (vitamin[r] * blend[p,r]) + vitamin['wheat'] * new_wheat_usage[p] <= max_vitamin[p] * production[p];